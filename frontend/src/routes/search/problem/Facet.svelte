...
<div class="rounded-xl bg-white px-6 py-4 shadow-md shadow-gray-300">
  ...
  {#each facet.category as part}
    <div class="flex flex-row items-center justify-between">
      <input id={part.label} type="checkbox" bind:group={categories} value={part.label} class="mx-1 inline-block cursor-pointer" />
  ...
  {#each facet.color as part}
    <div class="flex flex-row items-center justify-between">
      <input id={part.label} type="checkbox" bind:group={colors} value={part.label} class="mx-1 inline-block cursor-pointer" />
      <label for={part.label} class="mx-1 inline-block flex-grow cursor-pointer text-left">
        {part.label}
      </label>
      <label for={part.label} class="mx-1 inline-block cursor-pointer">
